...
namespace ExcelToJsonCs
{
	...
	{
		public static string GetApplicationRoot_()
		{
			var uri = new Uri(typeof(FileHelper).Assembly.Location);
			var exePath = Path.GetDirectoryName(uri.LocalPath);
			...
			{
				var appPathMatcher = new Regex(@"(?<!fil)/+[\S\s]*?(?=/+bin)");
				var appRoot = appPathMatcher.Match(exePath).Value;
				return appRoot;
			}

			throw new NotSupportedException(
				$"Not supported platform: {RuntimeInformation.OSDescription}"
			);
		}
